...
app = Flask(__name__)
CORS(app, resources={r"/api/*": {"origins": "https://smartrx.vercel.app"}})
...
# In-memory storage (replacing JSON files due to Vercel's ephemeral filesystem)
prescriptions = []
medications = []
reminders = []
alternatives_data = {}

# Folder configurations (for uploads, kept in memory or temporary)
...
def load_json(file_path, default=[]):
    return default  # No file-based storage; return default

def save_json(file_path, data):
    pass  # No file writes in Vercel
...
def upload_file():
    ...
    try:
        ...
        # Update in-memory prescriptions
        new_prescription = {
...
@app.route('/api/generate-prescription-doc', methods=['POST'])
...
def serve_doc(filename):
    return jsonify({"error": "Document serving not supported in serverless"}), 404
...
@app.route('/api/get-all-alternatives', methods=['GET'])
...
@app.route('/api/get-alternatives/<drug_name>', methods=['GET'])
